...
from thu_cli.sdk._literals import SERVER_NO, SERVER_YES, SSO_BAD_CREDENTIALS_MARKER
...
@pytest.mark.parametrize("s", ["yes", "y", "true", SERVER_YES])
def test_normalize_trust_device_str_yes(s: str):
    assert _normalize_trust_device(s)() is True


@pytest.mark.parametrize("s", ["no", "n", "false", SERVER_NO])
...
def test_extract_message_msg_note():
    html = f'<html><body><div id="msg_note">{SSO_BAD_CREDENTIALS_MARKER}</div></body></html>'
    assert _extract_message(html) == SSO_BAD_CREDENTIALS_MARKER
...
def test_dump_response_files_chmod_600(tmp_path: Path):
    """Regression: debug dump files should be mode 0600."""
    import requests
...
def test_auth_interaction_constructible_without_callbacks():
    """SDK users can construct ``AuthInteraction`` without callbacks."""
    a = AuthInteraction()
...
def test_resolve_password_caches_first_call():
    """``passwd_fn`` is called once and then cached."""
    calls = []
